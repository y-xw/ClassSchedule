...
//        getCaptcha();
    ...
    @Override
    public void loadCourseTimeAndTerm(final String xh, String pwd) {
        if (!verify(xh, pwd)) return;

        mImptView.showImpting();
        EduHttpUtils.newInstance().login(mSchoolUrl, xh, pwd, null, null,
    ...
    private boolean verify(String xh, String pwd) {
        if (mImptView == null) {
            //view被销毁
            return false;
        }
